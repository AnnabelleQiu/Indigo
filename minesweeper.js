...
class Cell {
    ...
    findBombs() {
        ...
        for (let x = -1; x < 2; x++) {
            for (let y = -1; y < 2; y++) {
                if (!(x == 0 && y == 0) && gameBoard.getCell(this.rowIndex + x, this.colIndex + y).hasBomb) {
                    bombs++;
                }
            }
        }
...
coverGrid.style.setProperty("grid-template-columns", "repeat(20, 30px)");
